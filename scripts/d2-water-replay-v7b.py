"""
EDNC Water Replay v7b — with debug to track which rows use exact vs fallback rates
"""
...
# Build per-meter-month rate table
meter_rates = {}
for (meter, unit), grp in data.groupby(["meter_str", "Unit umber"]):
    ...
    for month in sorted(grp["month"].unique()):
        ...
        if len(big) > 0:
            ...
            rates_by_month[month] = None
    if rates_by_month:
        meter_rates[(meter, unit)] = rates_by_month

fallback_counts = {"exact": 0, "meter_avg": 0, "meter_any": 0, "unit": 0, "standard": 0}

def get_rate(meter, unit, month, debug=False):
    key = (meter, unit)
    r = meter_rates.get(key, {})
    v = r.get(month)
    if v is not None:
    ...
    if vals:
        fallback_counts["meter_avg"] += 1
        return sum(vals) / len(vals)
    if r:
        for v in r.values():
            if v is not None:
                fallback_counts["meter_any"] += 1
                return v
    for k, kr in meter_rates.items():
        if k[1] == unit:
            for v in kr.values():
                if v is not None:
                    fallback_counts["unit"] += 1
                    return v
    fallback_counts["standard"] += 1
...
for _, row in data.iterrows():
    ...
    mc = ALT_MIN if unit in ALT_MIN_UNITS else MIN_CHARGE
    rate = get_rate(meter, unit, month)
...
print(f"Fallback: {fallback_counts}")
...
# Analyze MARGINAL rows
marginal = res_df[res_df["status"] == "MARGINAL"]
print(f"\nMARGINAL analysis ({len(marginal)} rows):")
print(f"  Avg cons: {marginal['cons'].mean():.2f}")
print(f"  Avg diff: {marginal['diff'].mean():.2f}")
# Check if they have exact rate or fallback
exact_marginal = sum(1 for _, row in marginal.iterrows() if meter_rates.get((str(row["meter"]), row["unit"]), {}).get(row["month"]) is not None)
print(f"  With exact per-meter-month rate: {exact_marginal}/{len(marginal)}")
