"""
EDNC Water Replay v7 — Per-meter rate lookup
"""
...
def get_rate(meter, unit, month):
    """Get rate for meter-unit-month."""
    key = (meter, unit)
    # Try exact meter-month
    r = meter_rates.get(key, {})
    v = r.get(month)
    if v is not None:
        return v
    # Try meter average across all months
    vals = [v for v in r.values() if v is not None]
    if vals:
        return sum(vals) / len(vals)
    # Try any other rate from this meter
    if r:
        for v in r.values():
            if v is not None:
                return v
