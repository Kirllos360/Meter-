"""
EDNC Water Replay v8 — Two-part formula: MC + first_unit_rate + main_rate * (cons-1)
"""
...
# Build per-meter-month main_rate table using two-part formula
# main_rate = (total - MC - first_unit_rate) / max(cons - 1, 1) for cons >= 2
meter_main_rates = {}
for (meter, unit), grp in data.groupby(["meter_str", "Unit umber"]):
    ...
    for month in sorted(grp["month"].unique()):
        ...
        big = mdf[mdf[cons_col] >= 2]  # need cons >= 2 to separate first-unit rate
        if len(big) > 0:
            r = big.iloc[0]
            c = r[cons_col]
            t = r[total_col]
...
def get_main_rate(meter, unit, month):
    """Get main_rate for meter-unit-month."""
    key = (meter, unit)
