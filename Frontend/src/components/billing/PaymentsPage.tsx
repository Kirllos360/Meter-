    ...
    <div>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={async () => {
              try {
                await apiPatch(`/payments/${selectedPayment.id}`, { notes: editNotes });
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          ...
          <p className="py-4 text-sm text-muted-foreground">Are you sure you want to delete payment <strong>{selectedPayment?.paymentNumber}</strong>? This action cannot be undone.</p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteDialogOpen(false)}>Cancel</Button>
            <Button variant="destructive" onClick={async () => {
              try {
                await apiDelete(`/payments/${selectedPayment.id}`);
