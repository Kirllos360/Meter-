...
CREATE OR REPLACE FUNCTION sim_system.normalize_arabic(text) RETURNS text AS $$ SELECT lower(regexp_replace(regexp_replace($1, '[\u0650-\u065F\u064B-\u0652]', '', 'g'), '[\u0625\u0623\u0622]', '\u0627', 'g')); $$ LANGUAGE SQL IMMUTABLE STRICT;
...
BEGIN
  ...
  SELECT * FROM (
    SELECT 'customer'::text, c.id::uuid, c.name::text, c.customer_code::text, ('customer:' || c.id)::text,
      greatest(
        CASE WHEN lower(c.name) LIKE lower('%' || search_query || '%') THEN 0.8 ELSE 0 END,
        CASE WHEN lower(c.customer_code) LIKE lower('%' || search_query || '%') THEN 1.0 ELSE 0 END,
        CASE WHEN c.phone LIKE '%' || search_query || '%' THEN 0.9 ELSE 0 END
      )::real as relevance
    ...
    FROM sim_system.meters m
    WHERE lower(m.serial_number) LIKE lower('%' || search_query || '%') OR lower(m.brand) LIKE lower('%' || search_query || '%')
    UNION ALL
    ...
    FROM sim_system.invoices i
    WHERE lower(i.invoice_number) LIKE lower('%' || search_query || '%')
    UNION ALL
    SELECT 'payment'::text, p.id::uuid, p.payment_number::text, p.status::text, ('payment:' || p.id)::text, 0.7::real
    FROM sim_system.payments p
    WHERE lower(p.payment_number) LIKE lower('%' || search_query || '%')
    UNION ALL
    ...
    FROM core.core_project pr
    WHERE lower(pr.name) LIKE lower('%' || search_query || '%') OR lower(pr.code) LIKE lower('%' || search_query || '%')
    UNION ALL
    SELECT 'ticket'::text, t.id::uuid, t.subject::text, t.status::text, ('ticket:' || t.id)::text, 0.6::real
    FROM sim_system.tickets t
    WHERE lower(t.subject) LIKE lower('%' || search_query || '%')
  ) sub
